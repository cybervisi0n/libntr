...
void WM_sp_init(WlInit *wlInit, WmInit *wmInit)
{
    ...
    wmspW.wm7buf = NULL;
    #ifdef SDK_PORT
    wmspW.status = malloc( sizeof( WMStatus ) );
    #else
    wmspW.status = NULL;
    #endif
    ...
    PXI_Init();
    PXI_SetFifoRecvCallback7(PXI_FIFO_TAG_WM, WmspPxiCallback);
...
{
    #ifdef SDK_PORT
    while (0 > PXI_SendWordByFifo7(PXI_FIFO_TAG_WM, (u64)ptr, FALSE))
        #else
...
BOOL WMSP_CheckMacAddress(const u8 *macAdr)
{
    ...
    if ((macAdr[0] == myMacAddress[0]) &&
        (macAdr[1] == myMacAddress[1]) &&
        (macAdr[2] == myMacAddress[2]) &&
        (macAdr[3] == myMacAddress[3]) &&
        (macAdr[4] == myMacAddress[4]) && (macAdr[5] == myMacAddress[5]))
    {
        return TRUE;
    }
    return FALSE;
...
void WMSP_CopyParentParam(WMGameInfo *gameInfop, WMParentParam *pparamp)
{
    ...
    gameInfop->attribute = (u8)(
        ((pparamp->entryFlag) ? WM_ATTR_FLAG_ENTRY : 0)
    ...
    gameInfop->childMaxSize = pparamp->childMaxSize;
...
u16 WMSP_GetAllowedChannel(u16 bitField)
{
    ...
    if (((max - center) < WMSP_ENABLE_CHANNEL_PERIOD) ||
        ((center - min) < WMSP_ENABLE_CHANNEL_PERIOD))
    {
...
#if 0
u16 WMSP_GetParentMaxSize(void)
{
WMStatus *status = WMSP_GetStatusStructure();
...
return (u16)(status->mp_maxSendSize - WM_HEADER_PARENT_MAX_SIZE);
}
else
{

return (u16)(status->mp_maxRecvSize - WM_HEADER_PARENT_MAX_SIZE);
}
}

u16 WMSP_GetChildMaxSize(void)
{
WMStatus *status = WMSP_GetStatusStructure();
...
return (u16)(status->mp_maxRecvSize - WM_HEADER_CHILD_MAX_SIZE);
}
else
{

return (u16)(status->mp_maxSendSize - WM_HEADER_CHILD_MAX_SIZE);
}
}

u16 WMSP_GetParentSize(void)
{
WMStatus *status = WMSP_GetStatusStructure();
...
return (u16)(status->mp_sendSize - WM_HEADER_PARENT_MAX_SIZE);
}
else
{

return (u16)(status->mp_recvSize - WM_HEADER_PARENT_MAX_SIZE);
}
}

u16 WMSP_GetChildSize(void)
{
WMStatus *status = WMSP_GetStatusStructure();
...
return (u16)(status->mp_recvSize - WM_HEADER_CHILD_MAX_SIZE);
}
else
{

return (u16)(status->mp_sendSize - WM_HEADER_CHILD_MAX_SIZE);
}
}
#endif

#ifndef SDK_BUILD_ARM
void WM_sp_SIM_init(void)
{
